...
public class Prism {
    ...
    public static ArrayList<ArrayList<Vertex2D>> OrderedFaces(ArrayList<ArrayList<Vertex3D>> faces3D, Vertex3D center) {
        ArrayList<ArrayList<Vertex3D>> copy = new ArrayList<>(faces3D);

        ArrayList<ArrayList<Vertex2D>> ordered = new ArrayList<>();
        while (ordered.size() < faces3D.size()) {
            ...
            ordered.add(to2D(copy.get(index), center));
            copy.remove(index);
        }
    ...
    public static void drawPrism(ArrayList<ArrayList<Vertex3D>> faces3D, Vertex3D center, int width, int height,
        ...
        ArrayList<ArrayList<Vertex2D>> faces2D = OrderedFaces(faces3D, center);

        for (int i = 0; i < faces2D.size(); i++) {
            ArrayList<Vertex2D> face = faces2D.get(i);
            ArrayList<Float> color = colors.get(i);
